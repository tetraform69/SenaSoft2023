<template>
    ...
    <div class="flex flex-row jusfity-center my-4">
        <div> 
            <select name="ruta" id="ruta" v-model="ruta_id" @change="obtenerGrafo" class="mx-4 rounded-md">
                <option value="0" selected disabled>Seleccione nodo incicial</option>
                <option
                    :value="item.id"
        ...
        <div class="flex">
            <button class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150" @click="obtenerGrafo">Obtener grafico</button>
        </div>

        <div class="flex">
            <button class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150" @click="obtenerGrafo">Obtene</button>
        </div>
    </div>
...
export default {
    ...
    methods: {
        obtenerGrafo(){
            axios.get(`/api/ruta?id=${this.ruta_id}`)
            .then(r=>{
                console.log(r.data)
